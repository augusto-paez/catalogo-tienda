...
export const Modal = {
  ...
  abrir(producto) {
    ...
    const imagen = producto.imagen
      ...
      : `<div class="modal-img-placeholder">${Catalogo.emojiPlaceholder()}</div>`;

    // Muestra la sección de variantes solo si el producto las tiene definidas
    const variantes = producto.variantes?.length
      ? `
        <p class="modal-talles-label">${producto.labelVariantes || "Variantes"}</p>
        <div class="modal-talles">
          ${producto.variantes.map(v => `<span class="modal-talle-chip">${v}</span>`).join("")}
        </div>
      `
      : "";
    ...
    window.location.hash = `producto-${producto.firestoreId}`;
  },
  ...
  // Registra los eventos de cierre y del botón de WhatsApp dentro del modal.
  // Como el contenido del modal se genera dinámicamente se usa delegación
  // de eventos en el overlay para no perder los listeners al re-renderizar.
  bindEventos() {
    const overlay = document.getElementById("modal-overlay");
    if (!overlay) return;

    overlay.addEventListener("click", e => {
      // Click en el fondo del overlay — cierra el modal
      if (e.target === overlay) {
        this.cerrar();
        return;
      }
      ...
      // Click en el botón de WhatsApp dentro del modal
      const btnWa = e.target.closest(".btn-wa-modal");
      if (btnWa && !btnWa.disabled && this.productoActual) {
        WhatsApp.consultarProducto(this.productoActual);
      }
    });
